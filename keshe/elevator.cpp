...
elevator::elevator(){
    type=nowNum=nowWeight=nextTime=flag=maxNum=maxWeight=0;
    nowFloor=1;
    for(int i=0;i<25;++i)
        can[i]=1;
}
...
bool elevator::add(int eFlag){//装载乘客
    if(nowNum==maxNum){
        can[nowFloor]=0;
        return 0;
    }
...
bool elevator::pop(int eFlag){//卸载乘客
...
bool elevator::haveBack(){//有前向返回要求
    if(flag==-1){
