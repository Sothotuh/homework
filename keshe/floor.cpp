...
void floor::add(passenger&temp){//增加一名乘客
	if(temp.to>myFloor)
		up.push_back(temp);
	else
		down.push_back(temp);
...
void floor::add(int weight,int to){//增加一名乘客
    if(to>myFloor)
...
passenger floor::pop(int upDown,int weight,int id,int eFlag){//弹出一名乘客
    if(upDown==-1){
...
bool floor::light(int flag,int id){//是否有等待
    if(flag==-1){
        if(can(id)==0)
            return 0;
        order(flag,id);
...
bool floor::order(int flag,int id){//预约
    if(flag==-1&&down.size()&&nextDown==0)
        return nextDown=id;
    if(flag==1&&up.size()&&nextUp==0)
        return nextUp=id;
    return 0;
}
